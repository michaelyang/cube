...
THREE.OutlinePass = function(resolution, scene, camera, selectedObjects) {
    ...
    this.renderTargetBlurBuffer1 = new THREE.WebGLRenderTarget(
        resx,
        resy,
        pars
    );
    ...
    this.renderTargetEdgeBuffer1 = new THREE.WebGLRenderTarget(
        resx,
        resy,
        pars
    );
            ...
            if (this.selectedObjects.length > 0) {
                ...
                if (this.pulsePeriod > 0) {
                    var scalar =
                        (1 + 0.25) / 2 +
                        (Math.cos(
                            (performance.now() * 0.01) / this.pulsePeriod
                        ) *
                            (1.0 - 0.25)) /
                            2;
